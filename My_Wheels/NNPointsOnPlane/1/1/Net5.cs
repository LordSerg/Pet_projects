...
namespace _1
{
    ...
    {//сеть (2+1)-(2+1)-1
        ...
        public static void Study(double in1, double in2, double out1)
        {
            ...
            n[4].IN = s[1].Weight * n[0].OUT + s[3].Weight * n[1].OUT + s[2].Weight * n[2].OUT;
            n[3].culc();
            n[4].culc();
            n[5].OUT = 1;
            n[6].IN = s[6].Weight * n[3].OUT + s[7].Weight * n[4].OUT + s[8].Weight * n[5].OUT;
            n[6].culc();
            Net_answer = Convert.ToInt32(n[6].OUT);//если OUt>0.5, то 1 иначе - 0
            squed_sum_of_errors += (out1 - n[6].OUT) * (out1 - n[6].OUT);
            //squed_sum_of_errors += (real_answer - Net_answer) * (real_answer - Net_answer);
            error = Math.Sqrt(squed_sum_of_errors / sets);
            //подсчет дельты
            n[6].DELTA = (out1 - n[6].OUT) * (1 - n[6].OUT) * n[6].OUT;//дельта выходного нейрона

            n[4].DELTA = s[7].Weight * n[6].DELTA * (1 - n[4].OUT) * n[4].OUT;//дельты скрытых нейронов
            n[3].DELTA = s[6].Weight * n[6].DELTA * (1 - n[3].OUT) * n[3].OUT;
            ...
            s[8].culc_gr(n[6].DELTA, n[5].OUT);
            s[7].culc_gr(n[6].DELTA, n[4].OUT);
            s[6].culc_gr(n[6].DELTA, n[3].OUT);
            s[5].culc_gr(n[4].DELTA, n[2].OUT);
            s[4].culc_gr(n[3].DELTA, n[2].OUT);
            s[3].culc_gr(n[4].DELTA, n[1].OUT);
